...
# NPU_PATCH: eagerly import torch_npu before caching is_npu_available.
# If TORCH_DEVICE_BACKEND_AUTOLOAD=0 was set at import time, torch_npu
# won't be auto-loaded by the backend system, causing is_torch_npu_available()
# to return False (torch.npu not yet attached). Importing torch_npu explicitly
# here ensures the module is loaded before the module-level cache is set.
# Without this, get_device_name() returns "cpu", and
# init_process_group(backend="cpu:gloo,cpu:nccl") raises ValueError.
try:
    import torch_npu  # noqa: F401  # ensure torch_npu is loaded
except ImportError:
    pass
...
def get_npu_versions() -> tuple[str, str]:
    ...
    # 使用 -i 0 查询第一个 NPU 设备（上游硬编码 -i 1 会在单卡/设备不可用时崩溃）
...
def is_support_ipc() -> bool:
    ...
    if is_npu_available:
        try:
            ...
            # npu-smi 命令执行失败 (设备不存在/驱动异常等), 不阻断训练
            logger.warning(f"npu-smi command failed (exit {e.returncode}), assuming no IPC support: {e}")
            return False
        except Exception as e:
            logger.warning(f"Error checking IPC support, assuming False: {e}")
            return False
