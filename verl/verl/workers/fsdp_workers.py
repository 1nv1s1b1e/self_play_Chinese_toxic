...
def _bind_npu_device_for_fsdp():
    """为 FSDP 工作进程正确绑定 NPU 设备.

    在 Ascend NPU 上, HCCL 需要每个 rank 绑定到不同的物理 NPU.
    Ray 的 NPU 加速器插件应自动设置 ASCEND_RT_VISIBLE_DEVICES,
    但如果未设置 (某些 Ray 版本或配置下), 则需要根据 LOCAL_RANK 手动绑定.
    """
    ...
    ascend_rt = os.environ.get("ASCEND_RT_VISIBLE_DEVICES")
    device_count = torch.npu.device_count()

    logger.info(
        f"[FSDP NPU 设备绑定] RANK={rank}, LOCAL_RANK={local_rank}, "
        f"ASCEND_RT_VISIBLE_DEVICES={ascend_rt}, npu.device_count={device_count}, "
        f"PID={os.getpid()}"
    )

    if ascend_rt is not None:
        # Ray 已隔离设备 (ASCEND_RT_VISIBLE_DEVICES 已设置)
        # 每个 worker 只能看到 1 张 NPU, device 0 就是被分配的那张
        device_id = 0
    else:
        # ASCEND_RT_VISIBLE_DEVICES 未设置, 所有 NPU 均可见
        # 使用 LOCAL_RANK 选择正确的物理 NPU
        device_id = local_rank % device_count
        logger.warning(
            f"[FSDP NPU] ASCEND_RT_VISIBLE_DEVICES 未设置! "
            f"使用 LOCAL_RANK={local_rank} -> NPU:{device_id} "
            f"(共 {device_count} 张 NPU 可见). "
            f"如果 HCCL 仍然报错, 请确认 Ray NPU 加速器插件是否正确配置."
        )

    get_torch_device().set_device(device_id)
    logger.info(f"[FSDP NPU] Rank {rank} 绑定到 NPU:{device_id}")
...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def __init__(self, config: DictConfig, role: str, **kwargs):
        ...
        if not torch.distributed.is_initialized():
            ...
            # 昇腾 NPU: 直接使用 hccl backend
            # 原始代码用 "cpu:gloo,npu:hccl" 混合 backend, 但在某些环境下
            # Gloo TCP connectFullMesh 会失败. 纯 hccl 已验证可以正常工作,
            # 且与 CriticWorker / RewardModelWorker 保持一致.
        ...
        # Ascend NPU: 在 HCCL 操作前绑定正确的设备
        _bind_npu_device_for_fsdp()
