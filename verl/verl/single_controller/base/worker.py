...
class Worker(WorkerHelper):
    ...
    def _setup_env_cuda_visible_devices(self):
        ...
        if is_ray_noset_visible_devices:
            ...
            if is_npu_available:
                # For NPU workers Ray sets ASCEND_RT_VISIBLE_DEVICES=<physical_id>,
                # which remaps that single physical card to **logical index 0**.
                # Two things must be kept consistent:
                #   1. set_device() must use logical index 0 (not physical_id)
                #   2. LOCAL_RANK must also be 0, because torch_npu / HCCL
                #      re-reads LOCAL_RANK during hcclCommInitRootInfoConfig
                #      and calls set_device(LOCAL_RANK) internally.
                #      If LOCAL_RANK == physical_id (e.g. 2) but only logical
                #      device 0 is visible, HCCL raises error code 5 / 107001.
