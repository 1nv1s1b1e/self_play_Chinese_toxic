...
__all__ = ["Worker", "ResourcePoolManager"]
...
class ResourcePoolManager:
    ...
    mapping: dict  # dict[Role, str], Role imported lazily to avoid circular dep
    ...
    def create_resource_pool(self):
        ...
        For FSDP backend, uses max_colocate_count=3: actor_critic_ref, rollout, reward model (optional).
...
class RayClassWithInitArgs(ClassWithInitArgs):
    ...
    def __call__(
        ...
        if use_gpu and device_name in ("cuda", "npu"):
            if device_name == "npu":
                # Ascend NPU: 不在 actor options 里请求任何 NPU/GPU 资源。
                # 原因: Ray 对自定义资源（NPU）使用分数值（1/max_colocate_count），
                # 在 worker 启动时 ray/runtime_context.py get_accelerator_ids_for_accelerator_resource
                # 会尝试把分数资源映射到设备 ID 列表，但自定义资源无 ID 映射 → IndexError。
                # 设备隔离完全依赖 _create_worker 注入的 ASCEND_RT_VISIBLE_DEVICES 环境变量，
                # 配合 PlacementGroupSchedulingStrategy 保证 worker 落在正确 bundle。
                pass
            else:
...
class RayWorkerGroup(WorkerGroup):
    ...
    def _create_worker(self, rank, pg_idx, pg, local_rank, resource_pool, ray_cls_with_init, worker_env, detached):
        ...
        # Ascend NPU: Without explicit ASCEND_RT_VISIBLE_DEVICES per worker,
        # Ray assigns "0" for ALL workers when using fractional resources
        # (NPU: 1/max_colocate_count), causing all workers to bind to the same
        # physical device → HCCL error code 5 during communicator initialization.
        #
        # runtime_env.env_vars are applied to os.environ when the worker PROCESS starts,
        # BEFORE any task execution code (including set_visible_accelerator_ids) runs.
        # Ray's set_visible_accelerator_ids() would then overwrite our value, BUT it
        # crashes with IndexError for fractional NPU resources (Ray bug). patch_ray_ascend.py
        # wraps that call in try-except so it returns early, leaving our value intact.
        # Each worker then imports torch_npu with the correct ASCEND_RT_VISIBLE_DEVICES,
        # isolating it to exactly one physical NPU → HCCL binds correctly.
        if self.device_name == "npu":
            env_vars["ASCEND_RT_VISIBLE_DEVICES"] = str(local_rank)
            # 关键: 必须允许 torch_npu 自动加载，否则 device.py 模块级缓存
            # is_npu_available=False → get_device_name()="cpu"
            # → init_process_group(backend="cpu:gloo,cpu:nccl") → ValueError
            env_vars["TORCH_DEVICE_BACKEND_AUTOLOAD"] = "1"
            # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
            # 关键: LOCAL_RANK 必须覆盖为 "0"
            #
            # 当 ASCEND_RT_VISIBLE_DEVICES=local_rank (例如 2) 时，该 worker
            # 只能看到 1 张物理 NPU，torch_npu 把它重映射为逻辑设备 0。
            # 但 HCCL 在 hcclCommInitRootInfoConfig 内部会重新读取 LOCAL_RANK，
            # 并调用 set_device(LOCAL_RANK)。若 LOCAL_RANK=2 而只有逻辑设备 0
            # 存在，set_device(2) 越界 → HCCL_E_PARA error code 5 / 107001。
            # 将 LOCAL_RANK 强制设为 "0"（逻辑设备索引），HCCL 调用 set_device(0)
            # 即可正确绑定至被 ASCEND_RT_VISIBLE_DEVICES 隔离出的那张物理 NPU。
            # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
            env_vars["LOCAL_RANK"] = "0"

            # ── 必须在 worker 级 runtime_env 中显式注入 HCCL 安全开关 ────────
            # worker 级 runtime_env 会覆盖 job 级 runtime_env（ray.init 注入的），
            # 因此 job 级设置的 HCCL_WHITELIST_DISABLE 等在 worker 内全部丢失。
            env_vars["HCCL_WHITELIST_DISABLE"]    = "1"
            env_vars["HCCL_SECURITY_INFO_DISABLE"] = "1"
            env_vars["HCCL_INTRA_ROCE_ENABLE"]     = "0"
            env_vars["HCCL_HOST_SOCKET_PORT_RANGE"] = "auto"
            env_vars["HCCL_NPU_SOCKET_PORT_RANGE"]  = "auto"
            # 从父进程继承探测到的内网 IP 和网口 (shell 脚本已探测)
            import os as _os
            for _k in (
                "HCCL_IF_IP", "HCCL_SOCKET_IFNAME",
        ...
        logging.info(
            f"[_create_worker] Creating worker '{name}': rank={rank}, local_rank={local_rank}, "
            f"pg_idx={pg_idx}, num_gpus={num_gpus}, device={self.device_name}, "
            f"master={self._master_addr}:{self._master_port}"
        )
