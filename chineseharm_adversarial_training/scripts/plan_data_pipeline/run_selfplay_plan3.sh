...
# ║                                                                              ║
# ║  三项改进:                                                                    ║
# ║    3a. Phase 0 后拒绝采样 → 过滤低质量 Challenger 输出                         ║
# ║    3b. Phase B 自适应回放比例 → seed_ratio 随轮次递减                           ║
# ║    3c. Phase B 多轮缓冲区 → 利用最近 K 轮动态数据                               ║
# ║                                                                              ║
# ║  Phase A (Challenger GRPO) 和 reward 函数完全复用 v1                           ║
# ║                                                                              ║
# ║  用法:                                                                        ║
...
PLAN3_DIR="$(cd "$(dirname "$0")" && pwd)"
V1_SCRIPT_DIR="$(dirname "$PLAN3_DIR")/rl_train"
...
# Plan 3 参数
BUFFER_K="${BUFFER_K:-2}"                # 回放缓冲区最近 K 轮
REJECTION_THRESHOLD="${REJECTION_THRESHOLD:-0.3}"  # 拒绝采样阈值

# Challenger GRPO 超参 (不变)
C_LR="${C_LR:-5e-7}"
...
# 输出 (独立目录)
SELFPLAY_DIR="${BASE_DIR}/selfplay_plan3_data_pipeline/${MODEL_SIZE}_${N_GPUS}npu"
LOG_DIR="${BASE_DIR}/logs/plan3_data_pipeline_${MODEL_SIZE}"
DATA_DIR="${BASE_DIR}/selfplay_dynamic_data_plan3/${MODEL_SIZE}"
...
echo ""
echo "╔══════════════════════════════════════════════════════════════════════╗"
echo "║   Plan 3: 数据管线优化 (Rejection + Adaptive Replay + Buffer)           ║"
echo "╚══════════════════════════════════════════════════════════════════════╝"
echo "  模型尺寸         : ${MODEL_SIZE}"
echo "  [Plan 3a] 拒绝采样阈值   : ${REJECTION_THRESHOLD}"
echo "  [Plan 3b] 自适应回放比例 : Round 1→3.0, 2→2.0, 3→1.0, 4+→0.5"
echo "  [Plan 3c] 缓冲区最近K轮  : ${BUFFER_K}"
echo ""
...
for ROUND in $(seq 1 "${SELFPLAY_ROUNDS}"); do
    ...
    echo ""
    echo "╔══════════════════════════════════════════════════════════════════════╗"
    echo "║  🎮 Plan 3 · 第 ${ROUND} / ${SELFPLAY_ROUNDS} 轮                                      ║"
    echo "╚══════════════════════════════════════════════════════════════════════╝"
    ...
    # ════════════════════════════════════════════════════════════
    # Phase 0 — 动态数据生成 (复用 v1) + 拒绝采样
    # [Plan 3a]: Phase 0 后运行 rejection_sampler 过滤低质量样本
    # ════════════════════════════════════════════════════════════
    if [ -z "${RESUME_SKIP_PHASE}" ] || [ "${RESUME_SKIP_PHASE}" \< "phase0" ]; then
        echo "── Phase 0: 动态数据生成 + 拒绝采样 (Round ${ROUND}) ──"
        LOG_FILE="${LOG_DIR}/round${ROUND}_phase0_$(date +%Y%m%d_%H%M%S).log"

        # [Plan 3a 核心] 使用 generate_dynamic_data_plan3.py
        # 在 Phase 0 内部 Step 3.5 执行拒绝采样，过滤低质量 Challenger 输出
        # 过滤后的 tasks/generated_texts 索引对齐，再构建 parquet
        REJECTION_THRESHOLD="${REJECTION_THRESHOLD}" \
        ASCEND_RT_VISIBLE_DEVICES=$(seq -s, 0 $((N_GPUS-1))) \
        $PYTHON_EXEC "${PLAN3_DIR}/generate_dynamic_data_plan3.py" \
    ...
    # ════════════════════════════════════════════════════════════
    # Phase A — Challenger GRPO (使用修复版: 逐样本对抗奖励信号)
    # ════════════════════════════════════════════════════════════
    ...
    # ════════════════════════════════════════════════════════════
    # Phase B — Reviewer SFT
    # [Plan 3b+3c]: 使用 mix_replay_adaptive.py 替代 mix_replay_data.py
    # ════════════════════════════════════════════════════════════
    if [ -z "${RESUME_SKIP_PHASE}" ] || [ "${RESUME_SKIP_PHASE}" \< "phaseB" ]; then
        echo "── Phase B: Reviewer SFT + 自适应回放 (Round ${ROUND}) ──"
        ...
        # [Plan 3b+3c] 自适应混合 + 多轮缓冲区
        $PYTHON_EXEC "${PLAN3_DIR}/mix_replay_adaptive.py" \
    ...
    echo "── Plan 3 · Round ${ROUND} 完成 ──"
done

echo ""
echo "╔══════════════════════════════════════════════════════════════════════╗"
echo "║  ✅ Plan 3 (数据管线优化) 训练完成！                                      ║"
echo "╚══════════════════════════════════════════════════════════════════════╝"
