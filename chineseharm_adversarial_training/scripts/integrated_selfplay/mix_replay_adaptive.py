...
自适应经验回放混合 — 统一版
============================
合并 Plan 3b (自适应回放比例) + Plan 3c (多轮缓冲区)。

3b. 自适应回放比例:
  seed_ratio 随轮次递减: Round 1→3.0, 2→2.0, 3→1.0, 4+→0.5

3c. 多轮缓冲区 (Replay Buffer):
  混合 "本轮新数据 + 最近 K 轮的动态数据 + 种子数据"
...
def parse_args():
    parser = argparse.ArgumentParser(description="自适应回放混合 (统一版)")
...
def convert_seed_to_grpo_format(df_seed: pd.DataFrame) -> pd.DataFrame:
    """将原始种子数据转为 Reviewer GRPO 所需的 prompt/reward_model 格式。"""
...
def main():
    ...
    print(f"🔄 [统一版] 自适应回放混合 (Round {round_idx})...")
    print(f"   - 动态新数据  : {args.dynamic_data}")
    print(f"   - 历史种子数据: {args.seed_data}")
    print(f"   - 自适应 seed_ratio = {seed_ratio}")
    print(f"   - 回放缓冲区 K = {args.buffer_k} 轮")
