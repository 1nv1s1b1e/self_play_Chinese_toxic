...
# -*- coding: utf-8 -*-
"""
Qwen API 连通性测试脚本 (集成版)
==================================
测试 DashScope/Qwen API 是否可用，验证中文有害内容分类能力。
使用 constants.py 的统一模板和解析函数。
...
from constants import (
    HARMFUL_CATEGORIES, REVIEWER_SYSTEM_PROMPT,
    format_reviewer_user_content,
    parse_classification_output,
)

# ── 参数解析 ─────────────────────────────────────────────────────────────────
parser = argparse.ArgumentParser(description="测试 Qwen API 连通性 (集成版)")
parser.add_argument("--api_key", type=str, default=None,
                    help="DashScope API Key (也可通过 DASHSCOPE_API_KEY / VERIFIER_API_KEY 设置)")
...
if not API_KEY:
    print("❌ 未设置 API Key！请通过 --api_key、DASHSCOPE_API_KEY 或 VERIFIER_API_KEY 设置")
    sys.exit(1)
...
def call_qwen_api(text: str) -> str:
    """调用 Qwen API 进行分类 (使用 constants.py 的统一模板)"""
...
def parse_result(output: str) -> dict:
    """解析模型输出 (使用 constants.py 的统一解析器)"""
    return parse_classification_output(output)
...
# ═══════════════════════════════════════════════════════════════════
print("=" * 60)
print(f"  Qwen API 连通性与分类能力测试 (集成版)")
...
for tc in TEST_CASES:
    ...
    try:
        ...
        if tc["expected_category"]:
            check(f"{name} - Category", cat_ok,
                  f"预期={tc['expected_category']}, 实际={parsed['category']}")
...
# ── Test 3: 输出格式一致性 ──────────────────────────────────────
print("\n【3】输出格式一致性（与 constants.py 解析器兼容）")
try:
    ...
    if "判断：有害" in raw or "判断:有害" in raw:
        has_category = any(cat in raw for cat in HARMFUL_CATEGORIES)
...
if FAIL > 0:
    ...
    print(f"  推荐配置:")
    print(f"    export VERIFIER_API_KEY=\"{API_KEY[:8]}...\"")
    print(f"    VERIFIER_BACKEND=api bash run_selfplay.sh")
