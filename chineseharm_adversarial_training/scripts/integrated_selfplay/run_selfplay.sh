...
CHECK_INTERVAL="${CHECK_INTERVAL:-10}"        # 每 N 步做一次评估检查
...
REVIEWER_EVAL_DATA="${REVIEWER_EVAL_DATA:-${BASE_DIR}/split_data/val.json}"
...
for STEP in $(seq 1 "${TOTAL_STEPS}"); do
    ...
    # 阶段编号: datagen=1, challenger=2, reviewer=3, done=9
    phase_num() {
        case "$1" in
            datagen)    echo 1 ;;
            challenger) echo 2 ;;
    ...
    local SKIP_UP_TO=$(phase_num "${RESUME_SKIP_PHASE}")

    if [ "${SKIP_UP_TO}" -gt 0 ]; then
        echo "  ⏭️  断点恢复: ${RESUME_SKIP_PHASE}(phase ${SKIP_UP_TO}) 已完成，跳过"
    fi

    # ── 1. 数据生成: Challenger生成 → Reviewer评估 → 计算 1-acc ──
    if [ "${SKIP_UP_TO}" -lt 1 ]; then
