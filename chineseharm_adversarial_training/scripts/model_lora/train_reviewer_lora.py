...
Reviewer使用messages格式（system/user/assistant）的chat数据
"""
...
def load_reviewer_sft_dataset(data_path: str, tokenizer, max_length: int = 2048):
    """
    加载并处理Reviewer SFT数据集
    支持两种格式:
    - messages格式: {"messages": [{"role":"system",...}, {"role":"user",...}, {"role":"assistant",...}]}
    - flat格式: {"instruction":"...", "output":"...", "category":"...", "original_text":"..."}
    只对assistant部分计算loss
    """
    ...
    if data_path.endswith('.jsonl'):
        ...
        for encoding in ['utf-8', 'gbk', 'gb18030', 'latin-1']:
            ...
            encoding = 'utf-8'
            print(f"  警告: 无法确定编码，使用 utf-8 + 容错模式")
        ...
        with open(data_path, 'r', encoding=encoding, errors='replace') as f:
            for line_no, line in enumerate(f, 1):
                ...
                try:
                    ...
                    skipped += 1
                    if json_errors <= 3:
                        print(f"  ⚠ 第{line_no}行 JSON解析错误: {e}")
        
        if json_errors > 0:
            print(f"  JSON解析失败: {json_errors} 条")
        
        # 如果JSONL全部失败，尝试回退到同目录下的parquet
        if not records:
            parquet_fallback = data_path.replace('.jsonl', '.parquet')
            if os.path.exists(parquet_fallback):
                print(f"  ⚠ JSONL全部解析失败，自动回退到 parquet: {parquet_fallback}")
                dataset = load_dataset('parquet', data_files=parquet_fallback, split='train')
            else:
                raise ValueError(
                    f"数据文件全部解析失败且无parquet备份: {data_path}\n"
                    f"  JSON错误: {json_errors} 条\n"
                    f"  请先运行: python scripts/create_data/fix_jsonl.py --file {data_path}\n"
                    f"  或重新生成: python scripts/create_data/prepare_all_data.py"
                )
        
        if dataset is None and records:
            # 诊断数据格式
            first_keys = set(records[0].keys())
            print(f"  首条数据字段: {first_keys}")
    ...
    def process_func(examples):
        """
        处理每个样本：使用chat template编码，只对assistant部分计算loss
        """
        model_inputs = {
            "input_ids": [],
            "attention_mask": [],
        ...
        for messages in items:
            ...
            prompt_messages = messages[:-1]  # 去掉assistant消息
    ...
    tokenized_dataset = dataset.map(
        ...
        desc="Tokenizing reviewer dataset"
    )
...
def main():
    parser = argparse.ArgumentParser(description="Reviewer LoRA训练")
    parser.add_argument("--model_path", type=str, 
                       default="/home/ma-user/work/test/models_base/Qwen/Qwen2.5-3B-Instruct",
                       help="基础模型路径")
    parser.add_argument("--data_path", type=str,
                       default="/home/ma-user/work/test/prepared_data/reviewer_sft/train.jsonl",
                       help="Reviewer SFT数据路径")
    parser.add_argument("--val_data_path", type=str,
                       default=None,
                       help="验证集数据路径(可选，会显示val loss)")
    parser.add_argument("--output_dir", type=str,
                       default="/home/ma-user/work/test/lora_models_toxicn/reviewer",
                       help="LoRA输出目录")
    ...
    parser.add_argument("--max_length", type=int, default=2048, 
                       help="最大序列长度（Reviewer需要较长上下文放rules）")
    ...
    print(f"Batch:    {args.batch_size} x {args.gradient_accumulation_steps} (累积)")
    ...
    # 加载数据集
    train_dataset = load_reviewer_sft_dataset(
        args.data_path, tokenizer, max_length=args.max_length
    )
    ...
    if args.val_data_path and os.path.exists(args.val_data_path):
        ...
        val_dataset = load_reviewer_sft_dataset(
            args.val_data_path, tokenizer, max_length=args.max_length
        )
    ...
    training_args = TrainingArguments(
        ...
        dataloader_num_workers=0,  # NPU环境下多worker易出错
        **ddp_kwargs,
    )
    ...
    if not use_multi or int(os.environ.get("LOCAL_RANK", 0)) == 0:
        ...
        print(f"\n✓ Reviewer LoRA训练完成！模型已保存到: {args.output_dir}")
