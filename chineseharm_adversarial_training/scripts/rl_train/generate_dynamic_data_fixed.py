...
# -*- coding: utf-8 -*-
"""
修复版 generate_dynamic_data — 注入逐样本对抗信号
==================================================

与 v1 generate_dynamic_data.py 的唯一区别:
...
v1 的缺陷:
  extra_info.verifier_asr = 类别级 float (同类别样本相同)
  → GRPO advantage 中被均值消去 → Challenger 无法学到对抗信号

修复:
  extra_info.reviewer_fooled = 逐样本 bool (每条样本独立)
  → GRPO advantage 中 fooled/not-fooled 产生真实梯度差异

本文件供所有 Plan 的 Phase 0 阶段使用 (替代 generate_dynamic_data.py)。
Plan 3 和 Plan 4 有自己的 Phase 0 脚本，但也集成了此修复。
"""
...
def main():
    """
    Phase 0 主流程，与 v1 逻辑相同，但 Step 7 使用修复版 parquet builder。
    """
    import argparse
    import random
    import logging
    ...
    logger = logging.getLogger("fixed_datagen")
    ...
    logger.info("╔══════════════════════════════════════════════════════════╗")
    logger.info(f"║  [Fixed] Phase 0: 动态对抗数据生成 (第 {args.round_idx} 轮)      ║")
    logger.info("╚══════════════════════════════════════════════════════════╝")
    logger.info(f"  [修复] Challenger parquet 将注入逐样本 reviewer_fooled 信号")
    ...
    logger.info(f"\n   [修复] 逐样本统计: {n_fooled}/{len(sample_rewards)} 条被 Reviewer 漏检 (fooled)")

    verifier.unload()

    # ════════════════════════════════════════════════════════════
    # Step 7: 构建 parquet — 修复版核心改动
    # 使用 build_challenger_parquet_adversarial 注入逐样本信号
    # ════════════════════════════════════════════════════════════
    logger.info("\n[Step 7] [修复] 构建含逐样本对抗信号的 Challenger GRPO parquet...")
    ...
    if len(challenger_df) > 0:
        ...
        if isinstance(sample_extra, dict):
            logger.info(f"   [验证] extra_info 含 reviewer_fooled={sample_extra.get('reviewer_fooled', 'MISSING')}")
            logger.info(f"   [验证] extra_info 含 reviewer_binary_ok={sample_extra.get('reviewer_binary_ok', 'MISSING')}")
    ...
    report = {
        ...
        "reward_fix": "per_sample_reviewer_fooled",
    ...
    logger.info(f"\n✅ [Fixed] Phase 0 完成！ ASR={overall_asr:.3f}, Fooled={n_fooled}/{len(sample_rewards)}")
